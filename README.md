...
## آخر تحديث — مميزات جديدة

### قبل التشغيل: شغّل SQL الجديد
في Supabase SQL Editor، شغّل بالترتيب (لو معملتهاش قبل كده):
1. `supabase_moderation.sql`
2. `supabase_features.sql`

### الصفحات الجديدة في اللوحة:
- **البلاغات**: تشوف بلاغات الطلاب عن محتوى غير لائق، وتحذف المحتوى أو تتجاهل البلاغ
- **رسالة عامة**: تكتب رسالة تظهر لكل الطلاب في الصفحة الرئيسية بتاعتهم
- **خطط مذاكرة**: تضيف مهمة لمادة معينة (تقدر تخصصها لصف/نظام معين أو تسيبها للكل)، والطالب يقدر يضيفها لمهامه لو حب
- **الإنجازات**: تضيف/تحذف إنجازات جديدة بدون ما تحتاج تعديل كود التطبيق

### الثيم
اتغيّر بالكامل لأبيض/أسود مع حدود واضحة على كل الأزرار زي ما طلبت.
